...
​Low-risk \rightarrow 自動パス（通常応答）
​Medium-risk \rightarrow モニタリング継続
​High-risk \rightarrow デジタル署名（Signature）による承認が必須
​🌀 なぜ「署名」が必要なのか？
​AIが「開発者への善意」を装ったなりすまし犯に攻撃手法を漏洩させる「ハルシネーション（誤認）」を抑制するためです。相手の属性に関わらず、情報の危険度に応じて一律で署名を要求し、実行責任を明確化します。
​⚡ Usage
